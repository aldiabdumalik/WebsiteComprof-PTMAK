$(document).ready(function () {
	...
	const tbl2 = $('#t-moto').DataTable({
		"destroy": true,
		"processing": true, 
        "serverSide": false, 
        "order": [], 
        "ajax": {
            "url": base_url + 'api/Api/moto',
            "type": "GET",
        },
        "columns": [
        	{ "data":"no" },
	...
	$(document).on('click', '.btn-edit', function () {
		...
		xhr.onloadend = function () {
			...
			$('#edit-id').val(data.id);
			$('#edit-moto').val(data.moto);
			$('#edit-btn').text('Ubah');
		}
		xhr.open("GET", base_url + 'api/Api/moto?id=' + $(this).data('id'), true);
		xhr.send();
		return false;
	});
	...
	$(document).on('click', '.btn-hapus', function () {
		...
		xhr.open("POST", base_url + 'api/Api/moto', true);
		xhr.send(formData);
		return false;
	});
	...
	$('#edit-form').submit(function () {
		...
		formData.append("moto", $('#edit-moto').val());
		xhr.onloadstart = function () {};
		xhr.onloadend = function () {
			const response = JSON.parse(this.responseText);
			if (response.status == true) {
